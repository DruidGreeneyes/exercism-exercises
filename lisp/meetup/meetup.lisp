...
(defun find-month-end (month year)
  (let ((mod-month (1+ (mod month 12))))
    (nth-value 3 (decode-universal-time
              (- (encode-universal-time 0 0 0 1 mod-month year) 1)))))

(defun get-month-list (month year)
  (loop for n from 1 to (find-month-end month year)
        collect n))
...
(defun filter-month (month year day-key)
  (let ((month-list (get-month-list month year))
